...
class Registry:
    def __init__(self):
        ...
        self.visual_accidents: List[dict] = []
    ...
    def add(self, r: CollRec) -> bool:
        ...
        if k in self._r:
            return False
        self._r[k] = r
        logger.info(f'[COLLISION] frame={r.frame} t={r.ts:.2f}s bayes={r.bayes:.0%} mahal={r.mahal:.1f} {r.msg}')
        return True
    ...
    def add_dl_accident(self, frame: int, ts: float, conf: float, bbox: Tuple[int, int, int, int]):
        ...
        for old in self.visual_accidents:
            ...
            if (_iou(old['bbox'], bbox) > 0.1 or dist < 250.0) and frame - old['frame'] < 10800:
                ...
                return
        self.visual_accidents.append({'frame': frame, 'ts': ts, 'conf': conf, 'bbox': bbox})
        logger.info(f'[VISUAL ACCIDENT] frame={frame} t={ts:.2f}s conf={conf:.0%} registered.')
    ...
    def save_report(self, path: str, states: Dict[int, VehState], gen_time: str):
        lines = ['COLLISION & ACCIDENT REPORT\n', f'Generated: {gen_time}\n', '\n', f'Total confirmed collisions (Physics-based): {len(self._r)}\n', f'Total confirmed accidents: {len(self.visual_accidents)}\n', f'Grand Total of Confirmed Incidents: {len(self._r) + len(self.visual_accidents)}\n', '\n', '=========================================\n', '--- PHYSICAL COLLISIONS (KINETIC) ---\n', '=========================================\n']
        ...
        lines += ['=========================================\n', '--- VISUAL DETECTED ACCIDENTS ---\n', '=========================================\n']
        for (i, r) in enumerate(self.visual_accidents, 1):
            lines += [f'#{i} VISUAL ACCIDENT ALERT\n', f"Time       : {r['ts']:.2f}s  (frame {r['frame']})\n", f"Confidence : {r['conf']:.0%}\n", f"BBox (xyxy): {r['bbox']}\n", '\n']
...
class BayesianFusion:
    ...
    LIKELIHOODS = {
        ...
        'convoy': (0.06, 0.68),
        'dl_accident': (0.88, 0.03)                                     
    }

    @classmethod
    def compute(cls, flags: Dict[str, float]) -> float:
        ...
        for (key, val) in flags.items():
            ...
            lc = val * pc + (1.0 - val) * (1.0 - pc)
            ln = val * pn + (1.0 - val) * (1.0 - pn)
...
class KinematicModel:
    ...
    @staticmethod
    def probabilistic_ttc(v1: dict, v2: dict, fps: float, mot: MotionAnalyzer, look_ahead_sec: float = 2.5) -> Tuple[float, Optional[float]]:
        ...
        for t in range(1, look + 1):
            ...
            d_t = np.linalg.norm(mu_d)
            if d_t <= 1e-06:
                prob = 1.0
            else:
                u = mu_d / d_t
                sigma_sq = float(u.T @ cov_d @ u)
                sigma = math.sqrt(max(1e-06, sigma_sq))
                
                if d_t <= thresh:
                    prob = 1.0
                else:
                    gap = d_t - thresh
                    prob = 0.5 * erfc(gap / (sigma * math.sqrt(2.0)))
            
            discounted_prob = prob * (gamma ** t)
            
            if discounted_prob > max_prob:
                max_prob = discounted_prob
                if prob >= 0.35 and ttc_sec is None:
                    ttc_sec = t / fps
                    
        return (max_prob, ttc_sec)
...
class RiskEngine:
    def __init__(self, fps: float, mot: MotionAnalyzer, W: int, H: int):
        ...
        self.clus_cd = Config.CLUSTER_CD
        self.static_obs = self._build_static_obs(W, H)
        self.visual_dets: List[dict] = []
    ...
    def _analyze_pair(self, v1: dict, v2: dict, fidx: int) -> Optional[dict]:
        ...
        (prob_ttc, ttc_s) = KinematicModel.probabilistic_ttc(v1, v2, self.fps, self.mot)
        
        if ttc_s is None and ttc_k is not None:
            ttc_s = ttc_k['secs']
        ...
        model_conf = 0.0
        if hasattr(self, 'visual_dets') and self.visual_dets:
            for d in self.visual_dets:
        ...
        flags = {
            ...
            'iou_med': iou >= 0.3,
            'contact': contact,
            'ttc_critical': prob_ttc if (ttc_s is not None and ttc_s <= Config.TTC_HIGH) else 0.0,
            'ttc_warning': prob_ttc if (ttc_s is not None and ttc_s <= Config.TTC_MED) else 0.0,
            'rel_spd_high': rel_spd >= self.crel_spd,
            'sudden_decel': v1.get('acc', 0.0) <= self.sud_dec or v2.get('acc', 0.0) <= self.sud_dec,
            'post_sig': post_sig,
            'impact_sig': impact_sig,
            'mahal_high': mahal_max >= Config.MAHAL_THRESH,
            'convoy': convoy,
            'dl_accident': model_conf                                                     
        }
        
        bayes = BayesianFusion.compute(flags)
        
        if both_slow and (mahal_max < Config.MAHAL_THRESH) and (v1.get('acc', 0.0) > self.sud_dec) and (v2.get('acc', 0.0) > self.sud_dec):
            bayes = min(bayes, 0.40)
            
        if rel_spd < 1.0 and (not impact_sig):
            bayes = min(bayes, 0.30)
            
        if bayes >= 0.35 and (not convoy):
            self.physics_trigger = True
            
        ev = {
            'iou': round(iou, 4), 'dist': round(dist, 2), 'gap': round(gap, 2),
            'contact': bool(contact), 'rel_spd': round(rel_spd, 3), 'hdiff': hdiff,
            'convoy': bool(convoy), 'both_slow': bool(both_slow), 'approach': approach,
            'ttc_secs': ttc_s, 'prob_ttc': round(prob_ttc, 4),
            'ttc_high': bool(ttc_s is not None and ttc_s <= Config.TTC_HIGH),
            'ttc_med': bool(ttc_s is not None and ttc_s <= Config.TTC_MED),
            'cf': ps['cf'], 'scf': ps['scf'], 'maxspd': round(ps['maxspd'], 3),
            'minacc': round(ps['minacc'], 3), 'impact_sig': bool(impact_sig),
            'post_sig': bool(post_sig), 'mahal_max': round(mahal_max, 3),
            'bayes': round(bayes, 4)
        }
    ...
    def _cluster(self, vehicles: List[dict], fidx: int) -> List[dict]:
        ...
        db = DBSCAN(eps=self.ceps, min_samples=Config.CLUSTER_MIN).fit(centers)
        labels = db.labels_
        ...
        for label in unique_labels:
            ...
            cluster_vids = [vehicles[idx]['id'] for idx in indices]
            evs.append({
                'level': 'MEDIUM',
                'type': 'CONGESTION',
                'ids': sorted(cluster_vids),
                'score': 50,
                'message': f"Congestion detected: {len(cluster_vids)} vehicles grouped",
                'color': (0, 165, 255)
            })
        return evs
