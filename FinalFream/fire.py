...
class FireDetector:
    ...
    def _load(self):
        ...
        try:
            ...
            self._loaded = True
            logger.info(f'[FireDetector] Loaded: {self.cfg.FIRE_MODEL}')
        except Exception as ex:
            logger.error(f'[FireDetector] Could not load fire model: {ex}')
    ...
    def analyze(self, frame: np.ndarray, vehicles: List[dict], states: Dict[int, any], fidx: int) -> List[dict]:
        ...
        for v in vehicles:
            ...
            if fidx % 5 != tid % 5:
                continue
            evaluated_keys.add(f'fire_veh_{tid}')
            evaluated_keys.add(f'smoke_veh_{tid}')
        ...
        if self.cfg.FIRE_FULL_FRM and fidx % self.cfg.FIRE_FULL_CD == 0:
            evaluated_keys.add('fire_global')
            evaluated_keys.add('smoke_global')
        ...
        for key in list(self._counters):
            if key in evaluated_keys and key not in fired_keys:
