...
# uploaded = files.upload() # Commented out as video.mp4 is already in /content/
...
class CalculateSpeed:
    ...
    def update(self, track_id, cx, cy):
        ...
        if track_id in self.prev_positions:
            ...
            speed = distance * self.fps  # pixels/sec
...
def analyze_frame(ids, coords, classes, car_speeds):
    decisions = []
    cars = []
    for i in range(len(ids)):
        if int(classes[i]) == CAR_CLASS_ID:
            cars.append({"id": int(ids[i]), "box": coords[i], "speed": car_speeds.get(int(ids[i]))})
...
frame_number = 0
speed_tracker = CalculateSpeed(cap.get(cv2.CAP_PROP_FPS))
...
while True:
    ...
    if boxes.id is not None:
        ...
        decisions = analyze_frame(ids, coords, classes, car_speeds)
