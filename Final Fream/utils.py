...
def ensure_dependencies():
    required_pkgs = {'fastapi': 'fastapi>=0.115.0', 'uvicorn': 'uvicorn[standard]>=0.30.0', 'multipart': 'python-multipart>=0.0.9', 'imageio_ffmpeg': 'imageio-ffmpeg>=0.5.1', 'sklearn': 'scikit-learn>=1.4.0', 'aiofiles': 'aiofiles>=23.2.1'}
    to_install = []
    ...
    if to_install:
        print(f'[DEPS] Installing missing requirements:{to_install}')
        try:
            subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-q', *to_install])
            print('[DEPS] Requirements installed successfully.')
        except Exception as e:
            print(f'[DEPS] Automatic requirements installation failed:{e}. Install manually from requirements.txt')
...
def setup_colab_tunnel(port: int, authtoken: str=None) -> str:
    try:
        try:
            ...
            print('[TUNNEL] Installing pyngrok...')
            subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-q', 'pyngrok>=7.2.0'])
            from pyngrok import ngrok
